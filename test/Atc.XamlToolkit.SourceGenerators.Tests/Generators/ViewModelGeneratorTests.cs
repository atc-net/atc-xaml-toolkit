...
public sealed partial class ViewModelGeneratorTests : GeneratorTestBase
{
    ...
    [Fact]
    public void NotifyDataErrorInfo_OnViewModelBaseDerivedClass_DoesNotEmitDiagnostic()
    {
        // ViewModelBase derives from ObservableValidator, so ValidateProperty
        // is in scope. No diagnostic.
        const string inputCode =
            """
            namespace TestNamespace;
        ...
        Assert.DoesNotContain(diagnostics, d => d.Id == "AtcXamlToolkit0009");
    }

    [Fact]
    public void NotifyDataErrorInfo_OnObservableObjectDerivedClass_EmitsDiagnostic()
    {
        // ObservableObject is INPC-only; ValidateProperty is not in scope,
        // so the generated setter would fail to compile. Surface diagnostic
        // 0009 up-front.
        const string inputCode =
            """
            namespace TestNamespace;
        ...
        Assert.Contains(diagnostics, d => d.Id == "AtcXamlToolkit0009");
    }

    [Fact]
    public void NotifyDataErrorInfo_OnINotifyPropertyChangedAttributedClass_EmitsDiagnostic()
    {
        // The [INotifyPropertyChanged] class-level attribute generates INPC
        // scaffolding but NOT ValidateProperty. The user combining it with
        // [NotifyDataErrorInfo] would otherwise see CS0103 inside the
        // generated file.
        const string inputCode =
            """
    ...
    [Fact]
    public void NotifyDataErrorInfo_OnObservableValidatorDerivedClass_DoesNotEmitDiagnostic()
    {
        // ObservableValidator directly — happy path.
        const string inputCode =
            """
            namespace TestNamespace;
    ...
    [Fact]
    public void NotifyDataErrorInfo_GeneratesValidatePropertyCallInSetter()
    {
        const string inputCode =
            """
            namespace TestNamespace;
    ...
    [Fact]
    public void ObservableProperty_WithoutNotifyDataErrorInfo_DoesNotEmitValidatePropertyCall()
    {
        // Pin the opt-in behaviour — without [NotifyDataErrorInfo] the
        // setter must NOT call ValidateProperty.
        const string inputCode =
            """
            namespace TestNamespace;
