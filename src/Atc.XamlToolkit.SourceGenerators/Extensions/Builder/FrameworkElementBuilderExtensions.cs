...
internal static class FrameworkElementBuilderExtensions
{
        ...
        FrameworkElementToGenerate frameworkElementToGenerate)
    {
        ...
        // Add using for BooleanBoxes when boolean DependencyProperty or AttachedProperty exists (WPF/WinUI only)
        if (frameworkElementToGenerate.XamlPlatform != XamlPlatform.Avalonia)
        {
            var hasBooleanDependencyProperty = frameworkElementToGenerate
                .DependencyPropertiesToGenerate?
                .Any(x => x.Type.StartsWith("bool", StringComparison.Ordinal)) == true;

            var hasBooleanAttachedProperty = frameworkElementToGenerate
                .AttachedPropertiesToGenerate?
                .Any(x => x.Type.StartsWith("bool", StringComparison.Ordinal)) == true;

            if (hasBooleanDependencyProperty || hasBooleanAttachedProperty)
            {
                builder.AppendLine("using Atc.XamlToolkit;");
            }
        }
